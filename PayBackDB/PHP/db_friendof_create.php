...
$newFriendID = filter_input(INPUT_POST, 'newFriendID',FILTER_VALIDATE_INT, $ID_options);
$newFriendEmail = $_POST['newFriendEmail'];
...
if (mysqli_connect_errno($link)) {
    ...
    $newFriendID = mysqli_real_escape_string($link, $newFriendID);
    $newFriendEmail = mysqli_real_escape_string($link, $newFriendEmail);
    ...
    // validate newFriendID
    if ($newFriendID != false) {
        $IDquery = mysqli_query($link, "SELECT `AccountID` FROM Account WHERE `AccountID` = '$newFriendID';");
        if (mysqli_fetch_object($IDquery)->AccountID != $newFriendID) {
            $newFriendID = false;
        }
    }
    // if invalid newFriendID, get newFriendID from newFriendEmail
    if ($newFriendID == false) {
        $IDquery = mysqli_query($link, "SELECT `AccountID`,`Email` FROM Account WHERE `Email` = '$newFriendEmail';");
        if (mysqli_num_rows($IDquery) == 1) {
            $newFriendID = mysqli_fetch_object($IDquery)->AccountID;
        }
        else if (mysqli_num_rows($IDquery) > 1) {
            $response["result"] = -6;
            $response["message"] = "Failed: New Friend Email '$newFriendEmail' matched >1 Account";
            $response["friendIDmatches"] = array();
            while ($row = mysqli_fetch_array($IDquery)) {
                // temp array
                $account = array();
                $account["NewFriendID"] = $row["AccountID"];
                $account["NewFriendEmail"] = $row["Email"];
                // push match into response array
            ...
            $response["result"] = -7;
            $response["message"] = "Failed: New Friend (ID: ".$_POST['newFriendID'].", Email: $newFriendEmail) matched no Account";
        }
    }
    //query to add transaction
    if (($response["result"] == null) && ($userID != false) && ($newFriendID != false)) {
        $newFriend = mysqli_query($link, "INSERT INTO `Friend_Of` (`UserID`,`FriendID`) VALUES ('$userID','$newFriendID');");
