...
if (mysqli_connect_errno($link)) {
    ...
    // query to select all notifications with receiveID == userID
    if (($userID != false)) {
        $select = mysqli_query($link, "SELECT `NoteID`,`SendID`,`Email`,`Fname`,`Lname`,`TransID`,`SendInfo`,`ReadFlag`,`NoteDate` FROM `Notification` JOIN `Account` ON `AccountID` = `SendID` WHERE `ReceiveID` = '$userID';");
        if ($select) {
            $response["result"] = 1;
            $response["quantity"] = mysqli_num_rows($select);
            ...
            while ($row = mysqli_fetch_array($select)) {
                ...
                $notif["Email"] = $row["Email"];
                $notif["Fname"] = $row["Fname"];
                $notif["Lname"] = $row["Lname"];
            ...
            $response["result"] = -7;
            $response["message"] = "Notification selection query failed for user '$userID'";
        }
    }
}
else {
