    ...
    if (($userID != false)) {
        if ($noteID > 0) {
            ...
            if (mysqli_fetch_object($notif)->ReceiveID == $userID) {
                if ($setFlag == "read") {
                    $update = mysqli_query($link, "UPDATE `Notification` SET `ReadFlag` = 1 WHERE (`ReceiveID` = '$userID') AND (`NoteID` = '$noteID');");
                    if ($update) {
                        $response["result"] = 1;
                        $response["message"] = "Notification '$noteID' for user '$userID' marked as READ";
                    }
                    else {
                        $response["result"] = -9;
                        $response["message"] = "Update Flag query failed for notification '$noteID' and user '$userID'";
                    }
                }
                elseif ($setFlag == "unread") {
                    $update = mysqli_query($link, "UPDATE `Notification` SET `ReadFlag` = 0 WHERE (`ReceiveID` = '$userID') AND (`NoteID` = '$noteID');");
                    if ($update) {
                        $response["result"] = 1;
                        $response["message"] = "Notification '$noteID' for user '$userID' marked as UNREAD";
                    }
                    else {
                        $response["result"] = -9;
                        $response["message"] = "Update Flag query failed for notification '$noteID' and user '$userID'";
