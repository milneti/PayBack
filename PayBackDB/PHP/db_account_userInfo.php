...
if (mysqli_connect_errno($link)) {
    ...
    if (($response["result"] == null) && ($userID != false)) {
        $friend = mysqli_query($link, "SELECT AccountID, Email, Fname, Lname FROM Account WHERE AccountID = '$userID';");
        if ($friend) {
            $response["result"] = 1;
            $response["message"] = "User info query success";
            $response["userInfo"] = array();
            $account = mysqli_fetch_object($friend);
            // push match into response array
            array_push($response["userInfo"], $account);
        }
        else {
            $response["result"] = -6;
            $response["message"] = "User info query failed";
        }
    }
}
else {
